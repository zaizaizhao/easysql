...
class TestContextBuilder:
    ...
    def test_build_context(self):
        ...
        # No code context provided: section renders empty and must not
        # leak an empty header into the prompt
        code_section = output.sections[-1]
        assert code_section.content == ""

    def test_build_context_with_code_context(self):
        """Code context enters the prompt through the section system."""
        from easysql.context import ContextBuilder, ContextInput
        ...
        assert "def get_patient(pid)" in output.user_prompt
        rendered = {s.name: s for s in output.sections}
        assert rendered["code_context"].content
        assert rendered["code_context"].token_count > 0
...
class TestSchemaSection:
    ...
    def test_list_format(self):
        ...
        from easysql.context import ContextInput, SchemaSection
...
class TestJoinPathSection:
    ...
    def test_render(self):
        ...
        from easysql.context import ContextInput, JoinPathSection
...
class TestCodeContextSection:
    """Test CodeContextSection class."""

    def test_render_empty_without_code_context(self):
        from easysql.context import ContextInput
        from easysql.context.sections import CodeContextSection

        result = create_test_retrieval_result()
        context = ContextInput(question="test", retrieval_result=result)

        content = CodeContextSection().render(context)

        assert content.name == "code_context"
        assert content.content == ""

    def test_render_with_code_context(self):
        from easysql.context import ContextInput
        from easysql.context.sections import CodeContextSection
        ...
        assert "SELECT 1" in content.content
...
class TestFewShotSection:
    ...
    def test_render_with_examples(self):
        ...
        from easysql.context import ContextInput, FewShotExample, FewShotSection
