...
class LLMFilter(TableFilter):
    ...
    def __init__(
        ...
        if not HAS_OPENAI:
            raise ImportError(
                "openai package is required for LLMFilter. "
    ...
    def _format_tables_info(
        self, tables: List[str], context: FilterContext
    ) -> str:
    ...
    def filter(
        ...
        if not self._api_key:
            return FilterResult(
                tables=tables,
                stats={"action": "skipped", "reason": "no API key"}
            )
        ...
        try:
            ...
            return FilterResult(
                tables=tables,
                stats={"action": "error", "error": str(e)}
            )
