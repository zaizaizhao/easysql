...
class McpExecutor(BaseSqlExecutor):
    ...
    def execute_sql(self, sql: str, db_name: str) -> ExecutionResult:
        result = self._run_async(self._execute_sql_async(sql, db_name))
        if isinstance(result, ExecutionResult):
            return result
        return ExecutionResult(success=False, error="Unexpected result type")
    ...
    def get_schema_info(self, db_name: str) -> SchemaInfoDict:
        result = self._run_async(self._get_schema_info_async(db_name))
        if isinstance(result, dict) and "tables" in result:
            return result  # type: ignore[return-value]
        return {"tables": [], "error": "Unexpected result type"}
